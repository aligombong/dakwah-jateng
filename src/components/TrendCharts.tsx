...
import { BarChart3, TrendingUp, Filter, Sparkles, Calendar } from 'lucide-react';
...
export const TrendCharts: React.FC<TrendChartsProps> = ({
  ...
  const [chartView, setChartView] = useState<'wilayah' | 'tren-periode'>('wilayah');
  ...
  const trendData = chronologicalPeriods.map((p) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        {/* Left: View Mode (Per Wilayah vs Tren Periode) */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 mb-4 border-b border-slate-100 gap-2">
            ...
            <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
              ...
              <span>
                {chartView === 'wilayah'
                  ...
                  : `Grafik Batang Tren ${getTrendTypeLabel()}: Kategori ${activeCategory.toUpperCase()}`}
              </span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              {chartView === 'wilayah'
                ? `Visualisasi komparatif data maqami untuk seluruh markaz Jawa Tengah (${periodeLabel})`
                : `Analisis pergerakan volume dakwah ${getTrendTypeLabel().toLowerCase()} dari waktu ke waktu`}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200">
              ...
              {chartView === 'wilayah' ? 'Data Periode Aktif' : `Mode: ${getTrendTypeLabel()}`}
            </span>
          </div>
        </div>
